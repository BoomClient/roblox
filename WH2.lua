...
_G.bossRange = 20  -- this is how far away the bot will try stay away from bosses
_G.mobRange = 40 -- this is how far away the bot will try to stay away from mobs
_G.ignoreAbilityRange = false
_G.smallTeleports = false -- if true, player teleports to places, might want to lower walkspeed alittle or else you'll probably get kicked
...
_G.webhookLink = "https://discord.com/api/webhooks/1164846575546277908/XpfIML8ukwnnMOi3Kllr16xZpo9_beazdVpugauYuP8N5k3Ft_rLN25CybrrDEkPnf43"
...
local gamer = {game:GetService("CollectionService"), game:GetService("PathfindingService"), table.insert, table.remove}
local sDebug = false
...
local me = plrs.LocalPlayer;
    ...
    warn('place id check')
    if game.PlaceId == 14363264964 or game.PlaceId == 3220968688 then	
        ...
        if mobType == "mob" or mobType == "ranged" or mobType == "melee" or mobType == "burly" then --f if mob
          if eggEvent then -- egg event only has one tag on each mob
            ...
            while (#cs:GetTags(instance) < 2) do -- normally each mob has two
              table.foreach(cs:GetTags(instance), print)
    ...
    local function webHookOnEnd()
      if _G.webhookEnabled then
        webhookLocals.timeLeft = game.Players.LocalPlayer.Character.Name
...
--[[
    ...
    workspace.ChildAdded:Connect(function(instance)
        ...
        elseif instance.Name == "firstBossCrystal" then 
          local precast = instance:WaitForChild("Union")

          local partHitBox = Instance.new("Part")
          partHitBox.Anchored = true
          partHitBox.CanCollide = false
          partHitBox.Transparency = 1
          partHitBox.CFrame = precast.CFrame
          partHitBox.Shape = Enum.PartType.Cylinder
          partHitBox.Size = Vector3.new(35, 150, 35)
          partHitBox.Name = "enemyRadius"
          partHitBox.Parent = workspace
          ...
          partHitBox:Destroy()
        elseif instance.Name == "firstBossCrystalDrop" then 
          local GoodSafeSpot 

          repeat
              for Index, SafeSpot in next, workspace.firstBossSafeZones:GetChildren() do
                  if SafeSpot.Transparency < 1 then
                      GoodSafeSpot = SafeSpot
                      break
                  end 
              end
          ...
          thePart.CFrame = CFrame.new(GoodSafeSpot.Position)
          repeat wait() until not Attack or not Attack:IsDescendantOf(workspace)
          forceObjectiveExists = false
        elseif instance.Name == "firstBossSpinningRockHitbox" then 
          local hitBox = instance:WaitForChild("hitBox")
          hitBox.Size = Vector3.new(0, 0, 0)
          
          repeat wait() until not workspace:FindFirstChild("firstBossCrystalDrop")

          hitBox.Size = Vector3.new(50, 50, 50)
        elseif instance.Name == "secondBossSpinningLaserHitbox" then 
          --[[local hitBox = instance:WaitForChild("hitBox")
        
          hitBox.Size = hitBox.Size + Vector3.new(0, 50, 16.5)

          local Nearest = EnemyManager:GetNearestEnemy()

          repeat 
              Nearest = EnemyManager:GetNearestEnemy()
              wait()
          until not Nearest or Nearest.Name ~= "Ancient Enchanted Tree"

          Attack:Destroy()]]
        elseif instance.Name == "firstBossFollowOrb" then 
          local innerBall = instance:WaitForChild("innerBall")
          ...
          partHitBox.Transparency = 0.5
          partHitBox.Name = "enemyRadius"
          partHitBox.Shape = Enum.PartType.Ball
          partHitBox.Size = Vector3.new(10, 10, 10)

          innerBall:GetPropertyChangedSignal("CFrame"):Connect(function()
              partHitBox.CFrame = innerBall.CFrame
          end)

          innerBall:GetPropertyChangedSignal("Size"):Connect(function()
              partHitBox.Size = Vector3.new(41, 41, 41)
              partHitBox.Parent = workspace
          end)
          ...
          partHitBox:Destroy() 
        elseif instance.Name == "battleMageOrb" then 
          local Origin =  instance.Position
          local MoveConnection 

          local partHitBox = Instance.new("Part")

          MoveConnection = instance:GetPropertyChangedSignal("Position"):Connect(function()
      ...
      local buildingSupplies = workspace:WaitForChild("firstBossSupplyModels"):WaitForChild("Model"):WaitForChild("buildingSupplies")

    buildingSupplies:GetPropertyChangedSignal("Transparency"):Connect(function()
        if buildingSupplies.Transparency == 0 then 
            local Nearest = findCloseEnemy()

            repeat
                local Closest
                local Distance = math.huge 

                for Index, Child in next, workspace.firstBossSupplyModels:GetChildren() do 
                    if Child:FindFirstChild("mainPart") then
                        local Magnitude = (Child.mainPart.Position - Player.Character.HumanoidRootPart.Position).Magnitude
                ...
                if Closest then
                    forceObjectiveExists = true
                    thePart.CFrame = CFrame.new(Closest.mainPart.Position)
                end

                Nearest = findCloseEnemy()

                wait()
            until Player.Character:FindFirstChild("firstBossSupplyOverhead") or not Nearest or Nearest.Name ~= "Crystal Golem"

            if Nearest and Nearest.Name == "Crystal Golem" then 
                repeat
                    local Closest
                    local Distance = math.huge 

                    for Index, Child in next, workspace.firstBossBuildZones:GetChildren() do 
                        if Child:FindFirstChild("beamPart") then 
                            local Magnitude = (Child.beamPart.Position - Player.Character.HumanoidRootPart.Position).Magnitude
                    ...
                    if Closest then 
                        thePart.CFrame = CFrame.new(Closest.beamPart.Position)
                    end

                    Nearest = findCloseEnemy()

                    wait()
                until Closest.built.Value == true or not Nearest or Nearest.Name ~= "Crystal Golem"

                if Nearest and Nearest.Name == "Crystal Golem" then
                    repeat 
                        Nearest = findCloseEnemy()
                        wait()
                    until buildingSupplies.Transparency == 1 or not Nearest or Nearest.Name ~= "Crystal Golem"
    ...
    Game:GetService("LogService").MessageOut:Connect(function(Message)
        --[[=table.foreach(strings, function(i,v)
          if Message == v then
              game:GetService('TeleportService'):Teleport(14363264964)
          end
      end)]]
    ...
    updatecheck = function()
      local places = {
        [14363263080] = { ['version'] = 46,   ['name'] = "Volcanic Chambers" },
        ...
        [14363264964] = { ['version'] = 4360, ['name'] = "Lobby"             },
    ...
    if game.PlaceId == 6216785535 then
      ...
      spawn(oceanFix)
    elseif game.PlaceId == 14363263080 then -- volcanic
